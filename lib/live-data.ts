...
export async function collectLiveDaily(previous?: DailyData | null, lastKnownGood?: DailyData | null): Promise<{ data: DailyData; sources: Record<string, string>; debug: LiveDebug }> {
  ...
  const etfRows = rowsOf(etf.json); const byDate = new Map<string, { totalFlow: number; validFlowCount: number; totalRows: number }>()
  for (const row of etfRows) { const rawDate = row.date ? String(row.date).slice(0, 10) : null; if (!rawDate) continue; const current = byDate.get(rawDate) ?? { totalFlow: 0, validFlowCount: 0, totalRows: 0 }; current.totalRows += 1; const flow = toNumber(row.flowUsd); if (flow != null) { current.totalFlow += flow; current.validFlowCount += 1 } byDate.set(rawDate, current) }
  ...
  const publishedDays = [...byDate.entries()].filter(([date, stats]) => date < todayNY && stats.validFlowCount > 0).sort(([a], [b]) => b.localeCompare(a))
  const publishedValues = publishedDays.map(([, stats]) => stats.totalFlow)
  const latestPublishedDate = publishedDays[0]?.[0] ?? null
  const latestStats = latestPublishedDate ? byDate.get(latestPublishedDate) : null
  ...
  const etfParsed = latestPublishedDate && latestStats ? { lastDay: latestStats.totalFlow, lastDayDate: latestPublishedDate, days7, days30, change30d: previous30 !== 0 ? ((days30 - previous30) / Math.abs(previous30)) * 100 : null } : null
