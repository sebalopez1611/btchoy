...
export async function collectLiveDaily(previous?: DailyData | null, lastKnownGood?: DailyData | null): Promise<{ data: DailyData; sources: Record<string, string>; debug: LiveDebug }> {
  ...
  const normalizeHistory = (rows: unknown, max?: number) => { const points = (Array.isArray(rows) ? rows : []).map((point: any) => ({ value: toNumber(point?.[1]), timestamp: typeof point?.[0] === 'number' ? new Date(point[0]).toISOString() : String(point?.[0] ?? '') })).filter((point): point is { value: number; timestamp: string } => point.value != null && Number.isFinite(new Date(point.timestamp).getTime())).sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()); if (!max || points.length <= max) return points; const step = (points.length - 1) / (max - 1); return Array.from({ length: max }, (_, index) => points[Math.round(index * step)]) }
  ...
  const historyFallback = (values: Array<{ value: number; timestamp: string }> | undefined, asOf: string | null | undefined) => isFresh(asOf, HISTORY_MAX_AGE_MS) && values?.length ? values : []
  const marketCapHistory = normalizeHistory(chartJson.market_caps)
  const volumeHistory = normalizeHistory(chartJson.total_volumes)
  ...
  const fearHistoryValues = normalizeHistory(((fearHistory.json as any)?.data ?? []).map((row: any) => [row.timestamp ?? row.time_until_update ?? row.ts, row.value]), 14)
  ...
  const sentimentOk = sentiment.httpStatus >= 200 && sentiment.httpStatus < 300
  const signalsOk = signals.httpStatus >= 200 && signals.httpStatus < 300
  const sentimentScore = sentimentOk ? toNumber(sentimentRow?.composite ?? sentimentRow?.compositeScore) : null
  const signalScore = signalsOk ? toNumber(signalData.composite) : null
  const score = sentimentScore ?? signalScore
  ...
  const liveBiasScoresValid = score != null && (sentimentScore != null || signalScore != null)
  ...
  const normalizedBias = liveBiasScoresValid ? { key: biasKey as any, label: biasLabel, news: scoreLabel(sentimentOk ? toNumber(sentimentRow?.newsScore ?? sentimentRow?.newsLayer?.score) : null, 'news'), institutional: scoreLabel(sentimentOk ? toNumber(sentimentRow?.institutionalScore ?? sentimentRow?.institutionalLayer?.score) : null, 'institutional'), traders: scoreLabel(sentimentOk ? toNumber(sentimentRow?.crowdScore ?? sentimentRow?.crowdLayer?.score) : null, 'crowd'), asOf: new Date().toISOString(), stale: false } : preservedBias
